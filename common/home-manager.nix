...
let
  ...
  mkUsers = hostPath:
    ...
    in listToAttrs (map (mkUser hostPath) users);
  ...
  mkUser = hostPath: name:
    let
      ...
      modulesConfigs =
        concatMap (src: import src flake) (filter pathExists modulesPaths);
...
in {
  home-manager = {
    ...
    extraSpecialArgs = removeAttrs specialArgs [ "modulesPath" ];
  };
}
