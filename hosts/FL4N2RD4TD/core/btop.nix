...
let
  mkValueString = v:
    ...
    let
      err = t: v:
        abort
        ("generators.mkBtopKV: " + "${t} not supported: ${toPretty { } v}");
    in if isInt v then
      toString v
...
in {
  xdg.configFile."btop/btop.conf".text =
    lib.generators.toKeyValue { inherit mkKeyValue; } {
      ...
      presets =
        "cpu:-6:default,proc:0:default cpu:0:default,mem:0:default,net:0:default cpu:0:block,net:0:tty";
      ...
      disks_filter =
        "/ /nix /nix/state /nix/store /mnt/emp /mnt/movies /mnt/music /mnt/redacted /mnt/shows";
