let
  ...
  mkHost = { self, hostsPath, system }:
    ...
    let
      ...
      paths = map (user: ../common/users/${user}) users
        ++ map (user: usersPath + /${user}) users
        ++ [ ../common/${platform} ../common/${platform}/${arch} fullHostPath ];
      ...
      modulesConfigs =
        concatMap (src: import src self) (filter pathExists modulesPaths);
